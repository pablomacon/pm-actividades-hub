...
function bindForm(form) {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      if (type === "text") { const codigo = form.elements.codigo.value.trim(); if (!codigo) throw new Error("Escribí el programa antes de entregarlo."); setFormMessage(form, "Guardando…"); await post("/entregas/guardar-texto", { numeroEjercicio, codigoTexto: codigo, respuestaExplicacion: explicacion }); }
      else { const file = form.querySelector('input[type=file]').files[0]; validateFile(file); setFormMessage(form, "Iniciando subida…"); const start = await post("/entregas/iniciar-upload", { numeroEjercicio, nombreOriginal: file.name, mimeType: file.type || "application/octet-stream", tamanioBytes: file.size }); setFormMessage(form, "Subiendo archivo a Drive…"); const metadata = await resumableUpload(start.uploadUrl, file, (percent) => setFormMessage(form, `Subiendo a Drive… ${percent}%`)); setFormMessage(form, "Confirmando entrega…"); await post("/entregas/confirmar-upload", { uploadToken: start.uploadToken, driveFileId: metadata.id, respuestaExplicacion: explicacion }); }
...
async function loadState() { const data = await post("/entregas/estado", {}); entregas = new Map((data.entregas || []).map((item) => [Number(item.numero_ejercicio), item])); }
